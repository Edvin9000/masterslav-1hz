...
export function circuitMarkup(single=false){
  const out=[];
  ...
  if(!single)out.push('<path d="M640 62 V350" class="stage-divider"/>');
  const stages=[[0,'M','d','clk','sm','rm','qm','qmb'],[640,'S','qm','clkb','ss','rs','qs','qsb']];
  for(const [dx,prefix,input,clock,s,r,q,qb]of (single?stages.slice(0,1):stages)){
    out.push(`<g transform="translate(${dx} 0)" class="latch-stage">`);
    text(320,40,single?'GATED D-LATCH':prefix==='M'?'MASTER':'SLAVE','section-label','middle');
    ...
    value(262,91,single?'S̅':prefix==='M'?'S̅m':'S̅s',s);
    value(262,355,single?'R̅':prefix==='M'?'R̅m':'R̅s',r);
    value(515,111,single?'Q':prefix==='M'?'Qm':'Qs',q);
    out.push('</g>');
  }
  if(single){wire('M40 420 H600','clk');dot(100,420,'clk');value(275,455,'E','clk');return out.join('');}
